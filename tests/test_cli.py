...
def test_scfile_no_args(runner, mock_convert_auto, mock_files_map):
    result = runner.invoke(scfile, [])
    assert result.exit_code == 0
    mock_convert_auto.assert_not_called()


def test_scfile_no_supported_files(runner, mock_convert_auto, mock_files_map):
    result = runner.invoke(scfile, ["path/to/nonexistent.file"])
    assert result.exit_code == 2
    mock_convert_auto.assert_not_called()


def test_scfile_with_invalid_model_formats(runner, mock_convert_auto, mock_files_map):
    result = runner.invoke(scfile, ["path/to/file.mcsa", "--model-formats", ".invalid", ".mcsa"])
    assert result.exit_code != 0
    mock_convert_auto.assert_not_called()


def test_scfile_with_invalid_output_path(runner, mock_convert_auto, mock_files_map):
