...
| 🗃️ **Архив**       | `.texarr`                                       | →   | `.zip`                   |
| ⛰️ **Регион**      | `.mdat`                                         | →   | `.mca`                   |
| 📄 **Документ**    | `itemnames.dat`, `common`,<br/>`prefs`, `sd0-4` | →   | `.json`                  |
...
> [!IMPORTANT]  
> **Обратная конвертация (`стандартный` в `игровой`) недоступна.**  
...
Выберите сборку на [странице Releases][releases]:

- **Windows:** `Setup` (рекомендуется) или `Portable`
- **Linux:** `Portable`

> Portable сборки распаковываются во временную папку при каждом запуске.
