...
GUI
~~~~
* Added Graphical interface (``PySide6``). Launches when arguments is empty.
...
Formats
~~~~~~~
* ``EfkmodelDecoder``: source format ``.efkmodel`` (geometry only).
* ``FbxEncoder``: output format ``.fbx`` (geometry only).
* ``MdatDecoder``: source format ``.mdat`` (world regions).
* ``McaEncoder``: output format ``.mca`` (anvil regions).
* ``McalDecoder``: source format ``.mcal`` (animation library, no export).
...
* ``GlbEncoder``: Meshes without skinning links when skeleton present.
