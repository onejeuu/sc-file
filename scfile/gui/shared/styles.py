...
class Styles:
    WINDOW = f"""
        QMainWindow, QDialog, QWidget {{ background-color: {Colors.BACKGROUND};  color: {Colors.TEXT}; font-family: "Segoe UI", sans-serif; }}
    ...
    CHECKBOX = f"""
        ...
        QCheckBox::indicator:disabled {{ border: 1px solid #252525; background: transparent; }}
        QCheckBox::indicator:checked {{ background: {Colors.ACCENT}; border: 1px solid {Colors.ACCENT}; }}
        QCheckBox::indicator:checked:disabled {{ background: #555; border: 1px solid #555; }}
    """
    ...
    RADIO = f"""
        ...
        QRadioButton::indicator:disabled {{ background: transparent; border: 1px solid #3d4455; }}
        QRadioButton::indicator:checked {{ background: {Colors.ACCENT}; border: 1px solid {Colors.ACCENT}; }}
        QRadioButton::indicator:checked:disabled {{ background: #555; border: 1px solid #555; }}
    """

    LIST = f"""
        QListWidget {{ background: {Colors.CARD}; color: {Colors.TEXT}; border: 1px solid #555; outline: none; font-size: 12px; }}
        QListWidget::item {{ padding: 3px 5px; }}
        QListWidget::item:selected {{ background: #3e4451; color: #ffffff; }}
    """

    COMBO = f"""
        QComboBox {{ background: {Colors.CARD}; border: 1px solid #555; padding: 2px 10px; }}
        QComboBox::drop-down {{ border: none; background: transparent; }}
        QComboBox QAbstractItemView {{ background: {Colors.CARD}; color: {Colors.TEXT}; border: 1px solid #555; outline: none; }}
        QComboBox QAbstractItemView::item {{ min-height: 20px; padding-left: 10px; border: none; }}
        QComboBox QAbstractItemView::item:selected {{ background-color: #3d4455; color: white; }}
    """
    ...
    INPUT = f"""
        QLineEdit {{ background: {Colors.BACKGROUND}; color: {Colors.TEXT}; border: 1px solid #555; padding: 4px; }}
    ...
    WARNING = "font-weight: medium; color: #e2a03f; font-size: 12px; line-height: 120%;"
