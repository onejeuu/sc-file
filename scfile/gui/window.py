...
FILE_TYPES = [
    FileType(
        "models", "Модели", [".mcsa", ".mcsb", ".mcvd"], features={"skeleton": "🦴 Скелет", "animation": "🌀 Анимация"}
    ),
    FileType("textures", "Текстуры", [".ol"]),
    FileType("images", "Изображения", [".mic"]),
    FileType("texarr", "Массив текстур", [".texarr"]),
    FileType("nbt", "NBT Данные", list(sorted(NBT_FILENAMES))),
]
...
class FileListWidget(QListWidget):
    ...
    def add_paths(self, paths: list[str]):
        for path in paths:
            if not path:
                continue
            item = QListWidgetItem(self._normalize_path(path))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.convert_btn.setStyleSheet("""
            QPushButton { background-color: #FFD666; color: black; font-weight: bold; border: none; }
            QPushButton:disabled { background-color: #444; color: #888; }
        """)

        right_column.addWidget(self.convert_btn)
